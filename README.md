# Gyeongguk — 기록형 풀이 MCP 서버

한국 변호사시험·경연 **기록형**(사건기록 → 법률서면) 풀이를 위한 [MCP](https://modelcontextprotocol.io) 서버.
사건기록 파일을 주면 **파싱 → 사실관계 → 청구추출 → 법리(쟁점·포섭격자) → 법령API 검증 → 작성 → 검토** 파이프라인을 단계별 파일로 진행한다.

기존 '기록형풀이' 룰스킬(개인 옵시디언 볼트)을 **독립 MCP 서버**로 이식한 것이다. 학습용 사례형 약점분석·카드·SRS는 의도적으로 제외하고, 사건기록 서면작성 파이프라인만 담는다.
